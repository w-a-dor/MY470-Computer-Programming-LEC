...
_Your cell here._
```{r}
scientists <- list('Alan Turing' = 'mathematician', 
...
print_professions <- function(dic) {
  for (name in names(dic)) {
    print(paste0(name, " was a ", dic[[name]], "."))
  }
}
...
is_integer <- lst == floor(lst)
ans <- ifelse(is_integer, lst^2, lst)
...
x <- rnorm(100)
positive <- x > 0
y <- ifelse(positive, "Positive", "Negative")
head(y)
```
